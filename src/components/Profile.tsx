...
export const Profile: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await setDoc(doc(db, 'users', user.uid), { displayName, photoURL, email: user.email, updatedAt: new Date() }, { merge: true });
  ...
  const fetchUserPosts = async ({ pageParam = null }: { pageParam?: any }) => {
    ...
    const posts: Post[] = snapshot.docs.map(doc => {
      ...
      return {
        ...
        text: data.text || '', publishedAt: data.createdAt?.toDate().toISOString() || new Date().toISOString(),
        media: data.media || [], likes: data.likes || [], commentsCount: data.commentsCount || 0, shares: 0, pinned: data.pinned || false,
      };
    });
        ...
        <div className="max-w-2xl mx-auto">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-6">
              ...
              <div>
                <h2 className="text-2xl font-bold text-[var(--text-primary)]">{displayName || 'Пользователь'}</h2>
