...
export const PollDisplay: React.FC<PollDisplayProps> = ({ pollId }) => {
  ...
  const handleLike = async () => {
    ...
    try {
      if (liked) {
        await updateDoc(pollRef, { likes: arrayRemove(currentUser.uid) });
        setLikes(prev => prev.filter(uid => uid !== currentUser.uid));
      } else {
        await updateDoc(pollRef, { likes: arrayUnion(currentUser.uid) });
    ...
    <>
      <div className="glass p-4 rounded-2xl space-y-3">
          ...
          <div className="flex-1">
            <Link to={`/profile/${poll.createdBy || '#'}`} className="hover:underline">
              <span className="font-semibold text-[var(--text-primary)]">
                {poll.authorName || 'Пользователь'}
              </span>
            </Link>
            <div className="text-xs text-[var(--text-secondary)]">
              {poll.createdAt?.toDate()?.toLocaleString() || 'Недавно'}
            </div>
          </div>
          <button
            ref={menuButtonRef}
            onClick={handleMenuToggle}
            className="text-[var(--text-secondary)] hover:text-[var(--text-primary)] p-1 rounded-lg hover:bg-white/10"
          >
        ...
        <div className="flex items-start gap-2">
          ...
          {userVoted && (
            <span className="text-xs text-green-400 ml-auto flex-shrink-0">✓ Проголосовал</span>
          )}
        </div>
          ...
          {(poll.options || []).map((option: any, index: number) => {
            ...
            return (
              <button
                ...
                className="w-full relative overflow-hidden rounded-xl bg-white/5 hover:bg-white/10 transition-colors p-3 text-left"
              >
                <div
                  className="absolute inset-0 bg-blue-500/20 transition-all duration-500 rounded-xl"
                  style={{ width: `${widthPercent}%` }}
                />
                <div className="relative flex items-center justify-between">
                  <span className="text-[var(--text-primary)] text-sm font-medium">
                    {option.text}
                    {isSelected && <span className="ml-2 text-xs text-blue-400">✓</span>}
                  </span>
        ...
        {/* Действия */}
        <div className="flex items-center gap-6 text-[var(--text-secondary)] pt-2 border-t border-[var(--border-color)]">
      ...
      {menuOpen && createPortal(
        <>
          ...
          >
            <button onClick={() => { navigator.clipboard?.writeText(`https://rite-messenger.vercel.app/poll/${poll.id}`); setMenuOpen(false); }} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-[var(--text-primary)] hover:bg-white/10">
            ...
            {isAuthor && (
              <>
                <button onClick={() => { setMenuOpen(false); }} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-[var(--text-primary)] hover:bg-white/10">
                  <Pin size={16} /> Закрепить
                </button>
                <button onClick={() => { handleDeletePoll(); setMenuOpen(false); }} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-red-400 hover:bg-white/10">
      ...
      {showComments && (
        <CommentsModal postId={pollId} onClose={() => setShowComments(false)} />
      )}

      {showReport && (
        <ReportModal postId={pollId} onClose={() => setShowReport(false)} />
      )}
    </>
  );
};